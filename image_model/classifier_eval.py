    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            for it in range(num_batches_per_epoch*100):
                ...
                if it %1000 == 0:
                    predictions = []
                    x_test = np.append(x_test, x_test[:mb_size], axis=0)
                    y_test = np.append(y_test, y_test[:mb_size], axis=0)
                    for jt in range(len_x_test//mb_size+1):
                        Xt_mb, Yt_mb = next_test_batch(jt, mb_size, x_test, y_test)
                        _, C_pred = sess.run([C_solver, C_real_logits], feed_dict={X: Xt_mb, Y: Yt_mb})
                        if len(predictions) == 0:
                            predictions = C_pred
                        else:
                            predictions = np.append(predictions,C_pred)
